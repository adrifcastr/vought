...
export async function run(message, args) {
    ...
    await message.guild.members.ban(member, { days: 7, reason: reason ? reason : 'no reason specified' }).catch(ex => { if(ex) {
    return message.channel.send('Couldn\'t ban this user. Please make sure that my role is higher then theirs and that they\'re not the guild owner.\nOtherwise Discord was unable to globally resolve the user.') }
    });
