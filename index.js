...
function LogCount() {
    manager.broadcastEval('this.guilds.cache').then(guilds => {
        ...
        Log(`Vought startup complete, \`${manager.shards.size}\` ${manager.shards.size != 1 ? 'shards' : 'shard'} and \`${guilds.length}\` guilds${guild_list.length < 1935 ? '\n```' + guild_list + '```' : ''}`);
    });
}
...
git.getLastCommit((err, commit) => {
    ...
    Log(`Vought starting, commit \`#${commit.shortHash}\` by \`${commit.committer.name}\`:\n\`${commit.subject}\``);
});